...
fn create_handle() -> *mut DeskLinkCoreHandle {
    ...
    assert_eq!(desklink_core_create(&mut handle), DESKLINK_CORE_STATUS_OK);
...
fn apply_ok(
    ...
    assert_eq!(
        desklink_core_apply(handle, event, &mut commands),
...
fn null_handles_and_null_outputs_are_rejected() {
    assert_eq!(
        desklink_core_create(null_mut()),
        DESKLINK_CORE_STATUS_INVALID_ARGUMENT
    );
    assert_eq!(
        desklink_core_destroy(null_mut()),
        DESKLINK_CORE_STATUS_INVALID_ARGUMENT
    );
    ...
    assert_eq!(
        desklink_core_apply(handle, std::ptr::null(), &mut commands),
        DESKLINK_CORE_STATUS_INVALID_ARGUMENT
    );
    assert_eq!(
        desklink_core_apply(handle, &event(DESKLINK_CORE_EVENT_START, 1), null_mut()),
        DESKLINK_CORE_STATUS_INVALID_ARGUMENT
    );
    assert_eq!(desklink_core_destroy(handle), DESKLINK_CORE_STATUS_OK);
}
...
fn unsupported_abi_version_is_rejected() {
    ...
    assert_eq!(
        desklink_core_apply(handle, &invalid, &mut commands),
        DESKLINK_CORE_STATUS_INVALID_ARGUMENT
    );
    assert_eq!(desklink_core_destroy(handle), DESKLINK_CORE_STATUS_OK);
}
...
fn invalid_event_kind_is_rejected() {
    ...
    assert_eq!(
        desklink_core_apply(handle, &event(u32::MAX, 1), &mut commands),
        DESKLINK_CORE_STATUS_INVALID_ARGUMENT
    );
    assert_eq!(desklink_core_destroy(handle), DESKLINK_CORE_STATUS_OK);
}
...
fn command_buffer_clear_zeroes_length_and_storage() {
    ...
    assert_eq!(
        desklink_core_command_buffer_clear(&mut commands),
        DESKLINK_CORE_STATUS_OK
    );
    assert_eq!(commands.len, 0);
    assert_eq!(commands.commands, [0; 8]);
    assert_eq!(
        desklink_core_command_buffer_clear(null_mut()),
        DESKLINK_CORE_STATUS_INVALID_ARGUMENT
    );
}
